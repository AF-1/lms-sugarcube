	...
	if ($step_counter > 2) {
		...
		if ($fire_day eq "YES") { # Should be active today, check firing hours
			...
			if ($hour >= $traf_from && $hour <= $traf_to) {
				get_traffic($client); # So active day and active hours so get Traffic report
			}
		}
...
sub get_traffic {
	...
	my ($traf_origin, $traf_destination, $traf_journey);
	...
	if ($traf_one_two_counter == 0) {
		...
		$traf_destination = $prefs->get('traf_destination'); # Traffic Destination
	} else {
		$traf_one_two_counter = 0;
		my $traf_enable_two = $prefs->get('traf_enable_two') || 0; # Traffic Address two ENABLED
		...
		if ($traf_enable_two == 1) {
			...
			$traf_destination = $prefs->get('traf_destination_two'); # Traffic Destination two
		} else {
			$traf_journey = $prefs->get('traf_journey_one'); # Traffic Name
			$traf_origin = $prefs->get('traf_origin'); # Traffic Origin
			$traf_destination = $prefs->get('traf_destination'); # Traffic Destination
	...
	my $url = 'https://maps.googleapis.com/maps/api/directions/json?origin=' . $traf_origin
	 . '&destination=' . $traf_destination
	 . '&mode=driving&departure_time=now&key=' . $traf_key;
...
sub got_traffic {
	...
	my $json_traffic = decode_json($data_set); # Decode RAW data into JSON format

	my $duration = $json_traffic->{routes}[0]{legs}[0]{duration}{text}; # Normal Journey Time
	my $duration_in_traffic = $json_traffic->{routes}[0]{legs}[0]{duration_in_traffic}{text}; # Time taking into account Traffic
	my $traffic_summary = $json_traffic->{routes}[0]{summary}; # Route Summary
	my $duration_in_traffic_secs = $json_traffic->{routes}[0]{legs}[0]{duration_in_traffic}{value}; # Time taking into account Traffic in secs
	...
	foreach my $player (Slim::Player::Client::clients()) { # Send alert to each player
		display_traffic ($player, $traffic_summary, $duration, $duration_in_traffic, $arr_time, $journey); # Send data to Display routine
	}
...
sub display_traffic {
	...
	my $duration_in_traffic = shift; # Time taking into account Traffic (text mins)
	my $arr_time = shift; # Estimated Arrival Time
	my $journey = shift; # Journey Name

	my $line1 = $journey . " via " . $traffic_summary;
	my $line2 = "Duration: " . $duration . " With Traffic: " . $duration_in_traffic;
